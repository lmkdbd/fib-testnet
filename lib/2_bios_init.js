...
describe('FIBOS BIOS 启动', () => {
	...
	it('update eosio.code to eosio.nft', () => {
		fibos.updateauthSync({
			account: 'eosio.nft',
			permission: 'active',
			parent: 'owner',
	...
	it("install eosio.nft", () => {
		fibos.setabiSync("eosio.nft", JSON.parse(fs.readFile(contracts_path + 'nft/nft.abi')), {
			authorization: 'eosio.nft'
		});
		fibos.setcodeSync("eosio.nft", 0, 0, fs.readFile(contracts_path + 'nft/nft.wasm'), {
			authorization: 'eosio.nft'
		});
        var c = fibos.getCodeSync('eosio.nft', true);
        assert.isNotNull(c.code_hash);
        c = fibos.getAbiSync('eosio.nft')
